...
function eMail(name, subject, message, received){
	this.name = name;
 ...
 var  createMailList = function(mailList) { 
	for(i = 0; i < mailList.length; i++){
		...
		var name = tempMail.childNodes[0].firstChild.nodeValue;
		var subj = tempMail.childNodes[1].firstChild.nodeValue;
		var rec = tempMail.childNodes[3].firstChild.nodeValue;
		var email = new eMail(name, subj, null, rec);
		inbox[i] = email;
	}
	for(i = 0; i < mailList.length; i++){
		var tempMail = mailList[i];
...
var showMessage = function(ev){
	...
	var currlist = getCurrentList();
		...
		text.innerHTML = currlist[id].message;
		if(currlist[id].read == false){
		currlist[id].read = true;
		if(CURR_MAIL_LIST == 3){
			for(i = 0; i < inbox.length; i++)
				if(inbox[i].subject == currlist[id].subject){
						inbox[i].read = true;
						break;
				}
		}
		...
		for(k = 0; k < unread.length; k++){
			if(currlist[id].subject == unread[k].subject){
				unread.splice(k,1);
			}
		}
...
var runSearch = function(id){
	...
	var currList = getCurrentList();
	var searchingTable = new Array();
	for(i = 0; i < currList.length; i++){
		var currmess = currList[i].message;
		var currmail = currList[i];
		if(currmess.match(pattern)){
			searchingTable.push(currmail);
		}
	}
	createTable(searchingTable);
}
